...
try:
    ...
    items = None
    logger.warning("items.py が見つかりません。")
...
if stripe.api_key == "sk_test_xxx":
    logger.warning("Stripe APIキーがデフォルト値のままです。本番環境では実際のキーを設定してください。")
...
try:
    ...
    logger.error(f"ページ設定に失敗しました: {e}")
    st.error(f"ページ設定に失敗しました: {e}")

# タイトル
try:
    st.title("VibeCore｜勝利の鼓動 × 勝ちの直感")
except Exception as e:
    logger.error(f"タイトルの設定に失敗しました: {e}")
    st.error(f"タイトルの設定に失敗しました: {e}")
...
try:
    ...
    logger.error(f"メニュー選択の設定に失敗しました: {e}")
    st.error(f"メニュー選択の設定に失敗しました: {e}")
    menu = "AI競馬予測"
...
try:
    ...
    logger.error(f"ユーザーカスタム設定の表示に失敗しました: {e}")
    st.sidebar.error(f"ユーザーカスタム設定の表示に失敗しました: {e}")
...
try:
    ...
    logger.error(f"メニュー内検索の処理に失敗しました: {e}")
    st.sidebar.error(f"メニュー内検索の処理に失敗しました: {e}")
...
try:
    ...
    logger.error(f"アクティビティ通知の表示に失敗しました: {e}")
    st.sidebar.error(f"アクティビティ通知の表示に失敗しました: {e}")
...
try:
    ...
    logger.error(f"メニュー選択時の演出に失敗しました: {e}")
    st.error(f"メニュー選択時の演出に失敗しました: {e}")
...
if menu == "AI競馬予測":
    try:
        ...
        logger.error(f"AI競馬予測セクションでエラーが発生しました: {e}")
        st.error(f"AI競馬予測セクションでエラーが発生しました: {e}")
...
# Stripe決済
    ...
    try:
        ...
        logger.error(f"Stripe決済セクションでエラーが発生しました: {e}")
        st.error(f"Stripe決済セクションでエラーが発生しました: {e}")
...
# 予想師コミュニティ
    ...
    try:
        ...
        logger.error(f"予想師コミュニティセクションでエラーが発生しました: {e}")
        st.error(f"予想師コミュニティセクションでエラーが発生しました: {e}")
...
# バトルパスチャレンジ
    ...
    try:
            ...
            if not mission["done"] and (mission["category"] == "無料" or st.session_state.battle_pass["premium"]):
                if st.button(f"ミッション達成：{mission['label']}（{mission['pt']}pt）", key=key):
                    try:
                        ...
                        logger.error(f"ミッション達成処理でエラーが発生しました: {e}")
                        st.error(f"ミッション達成処理でエラーが発生しました: {e}")
        ...
        logger.error(f"バトルパスチャレンジセクションでエラーが発生しました: {e}")
        st.error(f"バトルパスチャレンジセクションでエラーが発生しました: {e}")
    ...
    try:
        if items is None:
            ...
            try:
                ...
                st.error(f"items.py に display_items 関数が定義されていません: {e}")
            except Exception as e:
                st.error(f"商品一覧ページの読み込み中にエラーが発生しました：{e}")
    except Exception as e:
        logger.error(f"商品一覧セクションでエラーが発生しました: {e}")
        st.error(f"商品一覧セクションでエラーが発生しました: {e}")
...
# HEART残高と履歴
    ...
    try:
        ...
        logger.error(f"HEART残高と履歴セクションでエラーが発生しました: {e}")
        st.error(f"HEART残高と履歴セクションでエラーが発生しました: {e}")
...
# NFTコレクション
    ...
    try:
        ...
        logger.error(f"NFTコレクションセクションでエラーが発生しました: {e}")
        st.error(f"NFTコレクションセクションでエラーが発生しました: {e}")
...
# 管理者用ダッシュボード
    ...
    try:
        ...
        logger.error(f"管理ダッシュボードセクションでエラーが発生しました: {e}")
        st.error(f"管理ダッシュボードセクションでエラーが発生しました: {e}")
...
try:
    ...
    logger.error(f"CSSアニメーションの適用に失敗しました: {e}")
    st.error(f"CSSアニメーションの適用に失敗しました: {e}")

# Stripe Webhook エンドポイント
def handle_webhook():
    try:
        if "webhook" in st.experimental_get_query_params():
            ...
            if event["type"] == "checkout.session.completed":
                ...
                elif metadata["type"] == "heart":
                    amount = int(metadata["amount"])
                    st.session_state.heart_balance += amount
                    st.session_state.heart_history.append({
                        "action": f"HEART購入（{amount}HEART）",
                        "amount": amount,
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    })
                    update_heart_balance("user_123", st.session_state.heart_balance)
                    add_heart_history("user_123", f"HEART購入（{amount}HEART）", amount)
                    st.session_state.heart_history = get_heart_history()
                    st.session_state.payment_history.append({
                        "type": "HEART購入",
                        "amount": amount,
                ...
                elif metadata["type"] == "nft":
                    rarity = "ウルトラレア" if "ガチャ" in metadata["item"] and random.random() > 0.9 else "レア"
                    nft_item = {
                        "name": f"{metadata['item']}（{rarity}）",
                        "rarity": rarity,
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    }
                    st.session_state.nft_inventory.append(nft_item)
            ...
            st.success("決済が完了しました！")
    except Exception as e:
        logger.error(f"Webhook処理でエラーが発生しました: {e}")
        st.error(f"Webhook処理でエラーが発生しました: {e}")
